// ============================================
// USERS UTILS - Utilidades y guía de aprobación
// ============================================
    ...
    modal.addEventListener('click', (e) => {
        if (e.target === modal) {
            if (typeof closeFn === 'function') {
                ...
                // fallback: ocultar modal si no hay función de cierre
...
/**
 * 📋 GUÍA DE APROBACIÓN DE USUARIOS PARA PRODUCTOS MEDICINALES
 */
...
// Manejar clic fuera del modal de guía
function handleApprovalGuideClick(event) {
    if (event.target.id === 'approvalGuideModal') {
        closeApprovalGuide();
    }
}

// Exponer funciones globalmente
window.enableOverlayToClose = enableOverlayToClose;
window.showApprovalGuide = showApprovalGuide;
window.closeApprovalGuide = closeApprovalGuide;
window.handleApprovalGuideClick = handleApprovalGuideClick;

// Configurar modales cuando el DOM esté listo
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Nota: Los modales de usuarios (documentsModal, approveModal, rejectModal, editModal, addUserModal)
    // se manejan mediante event delegation en users-page.js, por lo que no es necesario
    // configurar overlays aquí para evitar listeners duplicados.
});
